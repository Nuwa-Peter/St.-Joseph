...
$sql = "SELECT s.id as stream_id, s.class_teacher_id, cl.name as class_name, s.n
ame as stream_name
        FROM streams s
...
if ($streams_with_teachers->num_rows > 0) {
    ...
    $check_sql = "SELECT 1 FROM attendances a JOIN stream_user su ON a.user_id =
 su.user_id WHERE su.stream_id = ? AND a.date = ? LIMIT 1";
 ...
 (?, ?, ?)";
    ...
    while ($stream = $streams_with_teachers->fetch_assoc()) {
        ...
        if ($result->num_rows === 0) {
            ...
            echo "Attendance not taken for $class_full_name. Sending reminder to
 teacher ID #$teacher_id.\n";

            $message = "Reminder: Please take attendance for your class, " . $cl
ass_full_name . ", for today.";
