...
// This script handles the submission of the class attendance form.
...
if (empty($stream_id) || empty($attendance_date) || empty($attendance_data)) {
    $_SESSION['class_attendance_error'] = "Invalid data submitted. Please select a class and mark attendance.";
    header("location: class_attendance.php");
    exit;
}
...
try {
    // 1. Get all student IDs for the given stream to ensure we only affect relevant students.
    ...
    // This makes the save operation idempotent and safer.
    ...
    $conn->commit();
    $_SESSION['class_attendance_success'] = "Attendance for " . htmlspecialchars($attendance_date) . " saved successfully!";

} catch (Exception $e) {
    $conn->rollback();
    $_SESSION['class_attendance_error'] = "An error occurred: " . $e->getMessage();
}
