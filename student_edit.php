...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Repopulate variables from POST
    $student_id = $_POST['id'];
    ...
    if (empty($errors)) {
        ...
        try {
            // Handle photo upload
            $photo_path = $student['photo']; // Keep old photo if new one isn't uploaded
            ...
            $sql_user = "UPDATE users SET first_name=?, last_name=?, lin=?, email=?, gender=?, phone_number=?, date_of_birth=?, student_type=?, photo=?, updated_at=NOW() WHERE id=?";
            $stmt_user = $conn->prepare($sql_user);
            $stmt_user->bind_param("sssssssssi", $first_name, $last_name, $lin, $email, $gender, $phone_number, $date_of_birth, $student_type, $photo_path, $student_id);
            $stmt_user->execute();
            $stmt_user->close();
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="stream_id" id="stream_id" class="form-select <?php echo isset($errors['stream_id']) ? 'is-invalid' : ''; ?>">
                        ...
                        <?php mysqli_data_seek($streams_result, 0); // Reset pointer ?>
